...
function Login() {
  ...
  return (
    <div className='w-full min-h-screen bg-[white] flex flex-col items-center justify-start'>
      ...
      <form className='w-[90%] max-w-[400px] h-[600px] md:shadow-xl bg-[#f3e4e4] rounded-2xl flex flex-col justify-center gap-[10px] p-[15px]' onSubmit={handleSignIn}>
            ...
            *{err}
            ...
            <button className='w-full h-[50px] rounded-full bg-[#24b2ff] mt-[30px] text-white cursor-pointer' disabled={loading}>{loading ? "Loading..." : "Sign In"}</button>
            <p className='text-center cursor-pointer' onClick={() => navigate('/signup')}>Don't have an account ? <span className='text-[#24b2ff] font-semibold cursor-pointer'>
                Sign Up
            </span>  </p>
      </form>
    </div>
